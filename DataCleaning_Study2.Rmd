---
title: "DataCleaning_Study2"
author: "Lu-K Gautier"
date: '2022-11-09'
output: html_document
---

This script is the data cleaning script for the project "Threat and Controllability" that study the impact of the controllability of a threat on attention processes. It use a spatial cueing paradigm.

The OSf link of this project is available at: https://osf.io/ynuc2/
...
# Aim of the script : 
Open and prepare data from the ThreatControllability project to run analysis.
...
- Ppt: The name of each participant
...
warning("Based on a factorial analysis on anxiety items, we couldn't agregate 'Calm' item to other anxiety items because this item don't load with the same factor.Moreover, despite items 'angry' and 'depressed' belong to the same factor as anxiety measurements, these 2 items have a lesser loading degree, so we decided not to agregate these items in our factorial score  according to our preregistration")
...
dtCtrl_Factor1 <- tmp %>%  
  ...
  select(Angry, Calm_Recod, Anxious, Nervous, Happy, Tense, Stressed, Depressed)
EFA_dt1_Ctrl <- factanal(dtCtrl_Factor1, factors=2, rotation = "promax")

dtThreat_Factor1 <- tmp %>%  
  filter(Block == "Threat") %>%
  select(Angry, Calm_Recod, Anxious, Nervous, Happy, Tense, Stressed, Depressed)
EFA_dt1_Threat <- factanal(dtThreat_Factor1, factors=2, rotation = "promax")

dtToon_Factor1 <-  tmp %>%  
  filter(Block == "Toon") %>%
  select(Angry, Calm_Recod, Anxious, Nervous, Happy, Tense, Stressed, Depressed)
EFA_dt1_Toon <- factanal(dtToon_Factor1, factors=2, rotation = "promax")
...
warning("In the threat block, the items 'Tense' seems to load to a lesser degree than other anxiety items.")
...
warning("Although there are some  variability in fit indices on these 3 CFA analysis using only the 4 anxiety items (Tense, Nervous, Anxious, Stressed), the model always show better indices (CFI, RMSEA, and SRMR) for the overall  dataset instead of separating each block individualy. Thus, we decided to calculate a factorial score for each participant without taking in consideration a differential factorial structure of this anxiety scale within each block of trials.")
...
EFA_dt1_Pretest <- factanal(dt1_Factor_Pretest, factors=2, rotation = "promax") # Promax = Oblique rotation
print(EFA_dt1_Pretest, digits = 2, cutoff = .3)

warning("Similarily to our first facorial analysis on anxiety scale, items 'angry' and 'depressed' belong to the same factor as anxiety measurements, but these 2 items show a lesser loading degree. So we decided not to agregate these items in our factorial score according to our preregistration. However, contrary to previous analysis, Calm item load on the same factor this time. Nevertheless, since loading of this item is lesser and in order to maintain coherence in our factorial score calculations, we decided to not agregate this item in the composite score.")
...
tmp <- dplyr::left_join(tmp, Anxiety_Scale, by= c("response_id","Block","Taskname","Task","Blockname"))


df_Final <- tmp

rm(list=setdiff(ls(), c("df", "df_Final", "data"))) # clear workspace except the data frames
...
df_Final <- tmp
rm(list=setdiff(ls(), c("df","data", "df_Final"))) # clear workspace except the data frame and the "Anxiety_Scale" dataframe
...
write.csv(df_Final, file = "Output/Transformed_Data/Final_df_ControllabilitySpatialCueing.csv")
